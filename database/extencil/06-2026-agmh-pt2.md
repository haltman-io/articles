...
The fast path is old boring Unix plumbing: run Tor locally, let it expose the default SOCKS listener on `127.0.0.1:9050`, then run AGMH through proxychains so the Python process and Git HTTPS subprocesses follow the same route.
...
The daemon normally listens on `127.0.0.1:9050` by default. If it does not, make it explicit in `/etc/tor/torrc`:
...
Test the GitHub API through Tor before moving mirrors. Use `--socks5-hostname` so DNS follows the SOCKS route too:
...
Keep `proxy_dns` enabled, choose `dynamic_chain` or `strict_chain`, and make the proxy list end with Tor:
...
Then move the mirrors through proxychains. Some Ubuntu installs expose both `proxychains` and `proxychains4`; others expose only the versioned command.
...
If only the versioned binary exists:
...
If you need the loud proxychains output for debugging, drop `-q`:

```bash
proxychains agmh run \
  --config agmh.config.toml \
  --verbose
```

If your AGMH install has SOCKS support in its Python dependency stack, the native flag can be used too:
...
The proxychains form is the faster compatibility fix because it wraps the process instead of betting everything on one library's proxy handling.
...
That was the point of AGMH before the tool had a proper release tag. Not heroics. Not a manifesto with no payload. Tor, proxychains, a mirror, and enough contempt for custody to move the work out.
